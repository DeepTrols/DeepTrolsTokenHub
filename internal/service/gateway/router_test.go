...
type mockChannelRepo struct {
	...
	findRoutePolicyFn func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error)
}
...
func (m *mockChannelRepo) FindRoutePolicy(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
	if m.findRoutePolicyFn != nil {
		return m.findRoutePolicyFn(ctx, tenantID, modelID, userLevel)
	}
	return nil, nil
}
...
func makeTestRoutePolicy(id uuid.UUID, modelID uuid.UUID, candidateIDs []uuid.UUID, fallback domain.FallbackPolicy, isActive bool, priority int) *domain.RoutePolicy {
...
func TestRoute_PolicyRestrictsToCandidateChannels(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	policyID := uuid.New()
	selectedChID := uuid.New()
	rejectedChID := uuid.New()
	instID := uuid.New()
	...
	channels := &mockChannelRepo{
		listByModelFn: func(ctx context.Context, modelID uuid.UUID, tenantID *uuid.UUID) ([]domain.Channel, error) {
			return []domain.Channel{
				makeTestChannel(selectedChID, model.ID, domain.ChannelStatusActive, 100, 100),
				makeTestChannel(rejectedChID, model.ID, domain.ChannelStatusActive, 100, 50),
			}, nil
		},
		listInstancesFn: func(ctx context.Context, channelID uuid.UUID) ([]domain.ChannelInstance, error) {
			return []domain.ChannelInstance{
				makeTestInstance(instID, channelID, "https://litellm.example.com", "openai/gpt-4o", 5),
			}, nil
		},
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			return makeTestRoutePolicy(policyID, model.ID, []uuid.UUID{selectedChID}, domain.FallbackDisabled, true, 10), nil
		},
	}
	router := NewRouter(models, channels)
	...
	if result.Channel.ID != selectedChID {
		t.Errorf("Channel.ID = %s, want %s (should select candidate channel)", result.Channel.ID, selectedChID)
	}
	if result.RoutePolicyID == nil || *result.RoutePolicyID != policyID {
		t.Errorf("RoutePolicyID = %v, want %s", result.RoutePolicyID, policyID)
	}
}

func TestRoute_PolicyFallbackDisabledReturnsError(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	policyID := uuid.New()
	...
	// Candidate channel ID that doesn't exist in the channel list
	nonExistentID := uuid.New()
	channels := &mockChannelRepo{
		listByModelFn: func(ctx context.Context, modelID uuid.UUID, tenantID *uuid.UUID) ([]domain.Channel, error) {
			return []domain.Channel{
				makeTestChannel(uuid.New(), model.ID, domain.ChannelStatusActive, 100, 100),
			}, nil
		},
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			return makeTestRoutePolicy(policyID, model.ID, []uuid.UUID{nonExistentID}, domain.FallbackDisabled, true, 10), nil
		},
	}
	router := NewRouter(models, channels)

	_, err := router.Route(context.Background(), makeIdentity(nil), "gpt-4o")
	if err != ErrNoChannelAvailable {
		t.Errorf("expected ErrNoChannelAvailable, got %v", err)
	}
}

func TestRoute_PolicyFallbackTenantDefault(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	policyID := uuid.New()
	availableChID := uuid.New()
	instID := uuid.New()
	...
	// Candidate list references a non-existent channel, but fallback=tenant_default
	// should bypass the candidate list and use all available channels.
	nonExistentID := uuid.New()
	channels := &mockChannelRepo{
		listByModelFn: func(ctx context.Context, modelID uuid.UUID, tenantID *uuid.UUID) ([]domain.Channel, error) {
			return []domain.Channel{
				makeTestChannel(availableChID, model.ID, domain.ChannelStatusActive, 100, 100),
			}, nil
		},
		listInstancesFn: func(ctx context.Context, channelID uuid.UUID) ([]domain.ChannelInstance, error) {
			return []domain.ChannelInstance{
		...
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			return makeTestRoutePolicy(policyID, model.ID, []uuid.UUID{nonExistentID}, domain.FallbackTenantDefault, true, 10), nil
		},
	}
	router := NewRouter(models, channels)
	...
	if result.Channel.ID != availableChID {
		t.Errorf("Channel.ID = %s, want %s (tenant_default should bypass candidate list)", result.Channel.ID, availableChID)
	}
}

func TestRoute_PolicyFallbackSharedAllowed(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	policyID := uuid.New()
	sharedChID := uuid.New()
	dedicatedChID := uuid.New()
	instID := uuid.New()
	...
	// Candidate list has a dedicated channel ID that doesn't exist in the list.
	// Fallback shared_allowed should include shared pool channels.
	nonExistentID := uuid.New()
	channels := &mockChannelRepo{
		listByModelFn: func(ctx context.Context, modelID uuid.UUID, tenantID *uuid.UUID) ([]domain.Channel, error) {
			return []domain.Channel{
				makeTestChannelWithPool(dedicatedChID, model.ID, domain.ChannelStatusActive, 100, 100, domain.PoolTypeDedicated),
				makeTestChannelWithPool(sharedChID, model.ID, domain.ChannelStatusActive, 100, 100, domain.PoolTypeShared),
			}, nil
		},
		listInstancesFn: func(ctx context.Context, channelID uuid.UUID) ([]domain.ChannelInstance, error) {
			return []domain.ChannelInstance{
		...
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			return makeTestRoutePolicy(policyID, model.ID, []uuid.UUID{nonExistentID}, domain.FallbackSharedAllowed, true, 10), nil
		},
	}
	router := NewRouter(models, channels)
	...
	// Should have selected the shared channel since candidate didn't match and fallback=shared_allowed
	if result.Channel.PoolType != domain.PoolTypeShared {
		t.Errorf("Channel.PoolType = %s, want %s (shared_allowed should select shared pool)", result.Channel.PoolType, domain.PoolTypeShared)
	}
}

func TestRoute_InactivePolicyIgnored(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	chID := uuid.New()
	instID := uuid.New()
	...
	channels := &mockChannelRepo{
		...
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			// Inactive policy with restrictive candidate list that doesn't include available channel
			return makeTestRoutePolicy(uuid.New(), model.ID, []uuid.UUID{uuid.New()}, domain.FallbackDisabled, false, 10), nil
		},
	}
	router := NewRouter(models, channels)
	...
	result, err := router.Route(context.Background(), makeIdentity(nil), "gpt-4o")
	if err != nil {
		t.Fatalf("unexpected error: inactive policy should be ignored: %v", err)
	}

	if result.Channel.ID != chID {
		t.Errorf("Channel.ID = %s, want %s (inactive policy should not restrict)", result.Channel.ID, chID)
	}
	if result.RoutePolicyID != nil {
		t.Errorf("RoutePolicyID = %v, want nil (inactive policy should not be applied)", result.RoutePolicyID)
	}
}

func TestRoute_NoPolicyFallsToOpenRouting(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	chID := uuid.New()
	instID := uuid.New()
	...
	if result.RoutePolicyID != nil {
		t.Errorf("RoutePolicyID = %v, want nil (no policy)", result.RoutePolicyID)
	}
}

func TestRoute_PolicyEmptyCandidateListMeansAllAllowed(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	policyID := uuid.New()
	chID := uuid.New()
	instID := uuid.New()
	...
	channels := &mockChannelRepo{
		...
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			return makeTestRoutePolicy(policyID, model.ID, nil, domain.FallbackDisabled, true, 10), nil
		},
	}
	router := NewRouter(models, channels)
	...
	if result.Channel.ID != chID {
		t.Errorf("Channel.ID = %s, want %s (empty candidate list = all allowed)", result.Channel.ID, chID)
	}
	if result.RoutePolicyID == nil || *result.RoutePolicyID != policyID {
		t.Errorf("RoutePolicyID = %v, want %s (policy should still be recorded)", result.RoutePolicyID, policyID)
	}
}

func TestRoute_RoutePolicyIDSetWhenPolicyApplied(t *testing.T) {
	model := makeTestModel("gpt-4o", domain.ModelStatusActive)
	policyID := uuid.New()
	chID := uuid.New()
	instID := uuid.New()
	...
	channels := &mockChannelRepo{
		...
		findRoutePolicyFn: func(ctx context.Context, tenantID *uuid.UUID, modelID uuid.UUID, userLevel string) (*domain.RoutePolicy, error) {
			return makeTestRoutePolicy(policyID, model.ID, []uuid.UUID{chID}, domain.FallbackDisabled, true, 10), nil
		},
	}
	router := NewRouter(models, channels)
	...
	if result.RoutePolicyID == nil {
		t.Fatal("RoutePolicyID is nil, want non-nil when policy is applied")
	}
	if *result.RoutePolicyID != policyID {
		t.Errorf("RoutePolicyID = %s, want %s", *result.RoutePolicyID, policyID)
	}
}
