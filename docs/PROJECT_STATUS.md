...
> 2026-09-02 全仓审计并立项企业化改造方案，见一百零六节）
...
| 3 | 企业/团队/预算体系 | 2026-08-25 收敛为单一开发者账号模型；**2026-09-02 重新开放**——企业客户体系（个人+企业，非 OEM）立项，见一百零六节与 `规划_平台定位与企业化改造方案.md` |
...
> 2026-09-02 起，企业化改造的实施基准以 `docs/规划_平台定位与企业化改造方案.md`
> 的 P0–P5 路线图为准（见一百零六节）。下表为改造立项前的历史建议，仅对账/监控/演练
> 等通用项仍与方案 P5 一致。
...
> 文档滞后。本次修正以代码为准：
>
> ⚠️ 2026-09-02 起本节状态已失效：OEM 体系随企业化改造整体冻结（管理入口移除、
> 数据保留），详见一百零七节。下表仅作冻结前的历史记录。
...
## 九十七、2026-08-30 重构目标验收（对齐 new-api 前后端）

> 对照本地 new-api 源码做最终验收：目标「照抄 new-api 重构前后端」按
> 工程化解读为——在我们自己的架构内吸收 new-api 的前端控制台与后端网关
> 能力面，并保留我方差异化（计费严谨性、对账、分组/折扣定价、双语）。

### 97.1 验收结论

- **能力覆盖**：new-api 全部核心路由组（控制台 / 网关 / 管理 / OAuth /
  公开接口）均已实现，见九十六节对齐矩阵；矩阵中标注「明确不做」的项均有
  产品决策或场景原因记录，属有意取舍而非遗漏；
- **质量**：Go 全量（真实 PG）、前端 42 文件 / 273 用例、e2e 6 passed
  全部通过；关键功能（OAuth、上传、计费快照、会话吊销、月度账单、L2、
  gzip）均经真实进程探针验证；
- **工程**：迁移 000021–000036 全部应用；工作区干净；历史已按功能提交
  （7b2a90d 大版本 + 各功能小提交）。

### 97.2 收尾状态

- 本目标达到验收标准，任务闭环；后续按需扩展项（如 2FA、Stripe、更多
  OAuth 源、Midjourney 等）由产品决策另行立项，不再列入本目标范围。
...
## 一百零六、2026-09-02 全仓审计与企业化改造方案立项

### 106.1 全仓审计结论

对后端（Go）、控制台前端（React）、`ai-nuxt` 落地页与基础设施进行全面审计：

- **规模**：Go 295 文件 / 67,203 行（123 个测试文件）；React 126 文件 / 18,209 行；
  73 个迁移；`go build ./...` 通过，非测试代码无 TODO/FIXME 遗留。
- **质量**：计费引擎（Reserve-Settle/Commit-Release + 乐观锁 + 幂等 + 全链路
  `shopspring/decimal`）、5 不变量证据链、安全基线（生产配置 fail-fast、httpOnly Cookie、
  限流、审计）达到生产级。
- **发现的问题**：
  - B2 账外注资（注册赠送裸写 `wallets`）仍未修复；
  - 企业管理只有数据层（`tenants`/`tenant_memberships`/`tenant_invitations`），
    自服务 API 全部缺失；注册 `user_type` 硬编码 `personal`；
  - 平台权限仅 `users.role: user|admin` 两态，无细粒度角色；
  - 支付依赖易支付（epay），官方支付宝/微信未接；
  - `ai-nuxt` 落地页无版本控制，认证表单为静态原型，定价为静态 JSON 快照。
- **关键利好**：网关租户解析已脱离 Host 域名（改为账号 membership 解析），单域名
  部署模式成立，去 OEM 化的最大技术风险已消除。

### 106.2 立项《平台定位与企业化改造方案》

新增 `docs/规划_平台定位与企业化改造方案.md`（v1.0，473 行 / 11 节），作为后续实施基准：

- **定位收敛**：单一运营方平台，只面向个人用户 + 企业用户，不做多渠道 / OEM。
- **语义重定义**：`tenants` 重定义为企业客户；OEM 能力（自定义域名 / 品牌 / 选品 /
  结算配置 / 租户级定价）冻结，数据表保留不删。
- **已拍板决策**（详见方案 §9）：企业共享钱包（独立 `enterprise_wallets` 表，仅改网关
  钱包解析入口）；管理后台动态可配置角色（固定权限目录 + 自由角色组合）；成员月度消费
  上限一期实现（Redis 微元计数 + DB 降级 + fail-closed）；支付宝 / 微信自接并移除
  epay；一期纯人工企业审核；一期全平台统一定价；对公转账 / 退款列二期。
- **路线图**：P0 边界冻结与安全网 → P1 支付自接 → P2 企业管理 MVP → P3 动态 RBAC
  → P4 落地页对接 → P5 生产化；核心路径遵循既有 TDD 纪律。

### 106.3 本方案取代的既有决策

- **取代 3.2 节第 3 项**（2026-08-25「企业/团队/预算体系收敛为单一开发者账号模型」）：
  重新开放企业客户体系（个人 + 企业），但不恢复 OEM 多渠道。
- **取代第五节「建议实施计划」**：企业化改造以方案 P0–P5 路线图为准。
- **取代 3.4 节「支付集成列 MVP 范围外」**：支付宝 / 微信直连纳入一期（P1）。

### 106.4 验证

- 方案文档已写入 `docs/规划_平台定位与企业化改造方案.md`；README 文档表与本报告
  3.2 / 五节已同步指引。
- 本次为纯规划，未改动任何代码。
...
## 一百一十六、2026-09-03 [P0.5] 备份恢复演练（TH-P05-07）

B7 缺口"恢复从未演练过"收口：构建可重复演练工具链并真实执行两轮
完整演练，全部 PASS；Batch 1 审计缺口 C-5（manifest/restore 验证必须
检查 schema 迁移版本）一并收口。

### 116.1 工具链

- `scripts/restore_drill.sh`：目标防护（生产形态库名/无隔离标记一律
  拒绝，退出码 2）→ DROP+CREATE 隔离目标库 → `pg_restore`（计时）→
  验证（迁移标记对象 + schema 对象/列数指纹、manifest 行数、钱包
  不变量 W1/W2、7 表逐行校验和、分状态支付金额）→ 报告。
- `scripts/seed_drill_fixture.sh`：隔离库播种"真实的钱与证据"夹具
  （完整钱包生命周期 + 支付订单 4 状态 + 三类 usage_source 证据），
  播种后自校验钱包不变量，使演练在非空账本上验证。
- `docs/DEPLOYMENT.md` §5.3：演练操作步骤、失败处理表（按退出码）、
  "生产恢复必须先经演练"约束。

### 116.2 实测结果（两轮）

- Run 1（主库基线）：110199 bytes 备份；恢复 0s；迁移标记 + 39 表
  指纹、7 表行数、校验和全过；W2 以源基线对比忠实暴露主库 1 个既有
  钱包异常（有余额无流水），恢复未引入新违例。
- Run 2（非空账本夹具）：13 笔账本（含 settle 低估/超额、commit、
  release、transfer、未完结 reserve 挂账）；恢复 0s；全部验证项通过；
  抽样 3 钱包 `balance == 账本净额` 精确相等。
- 失败注入 4 例（生产目标 / 无标记目标 / 缺 dump / 缺 manifest）全部
  按预期拒绝或失败。

### 116.3 架构事实与遗留

- 本项目无 `schema_migrations` 版本表（迁移按文件名顺序裸 SQL 应用）；
  "迁移版本"以标记对象 + 对象集合指纹验证，新增迁移须同步更新
