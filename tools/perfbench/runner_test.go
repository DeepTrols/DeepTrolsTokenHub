...
func TestRunConcurrentLoadUsesAuthenticatedUniqueRequests(t *testing.T) {
	...
	result, err := perfbench.Run(context.Background(), perfbench.Config{
...
func TestRunRejectsAmbiguousLoadConfiguration(t *testing.T) {
	...
	_, err := perfbench.Run(context.Background(), perfbench.Config{
...
func TestRunFixedRateStopsSchedulingAtDeadline(t *testing.T) {
	...
	result, err := perfbench.Run(context.Background(), perfbench.Config{BaseURL: server.URL, Protocol: perfbench.ProtocolChat, Mode: perfbench.ModeRate, Rate: 1, Duration: 100 * time.Millisecond, Timeout: time.Second})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunThroughputIncludesOutstandingRequestDrainTime(t *testing.T) {
	...
	result, err := perfbench.Run(context.Background(), perfbench.Config{BaseURL: server.URL, Protocol: perfbench.ProtocolChat, Mode: perfbench.ModeConcurrency, Concurrency: 1, Duration: 20 * time.Millisecond, Timeout: time.Second})
	if err != nil {
		t.Fatal(err)
	}
