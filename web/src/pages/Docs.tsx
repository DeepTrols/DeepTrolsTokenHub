...
function ModelsSection() {
  const { data: modelsData, isLoading, isError, error } = useQuery<ModelInfo[]>({ queryKey: ["gateway", "models", "demo"], queryFn: async () => { const res = await fetch("/v1/models", { headers: { "Content-Type": "application/json", Authorization: "Bearer demo" } }); if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error((err as { error?: { message?: string } }).error?.message || "Failed"); } const d = await res.json() as { data?: ModelInfo[] }; return d.data || []; } });
