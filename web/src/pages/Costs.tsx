...
export default function Costs(){
  ...
  return <div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
      {[{l:"总调用",v:t.r.toLocaleString()},{l:"总消耗",v:formatAmount(t.f)+" CNY"},{l:"上游成本",v:formatAmount(t.u)+" CNY",c:"text-orange-500"},{l:"利润",v:formatAmount(tp)+" CNY · "+tm.toFixed(1)+"%",c:tp>=0?"text-emerald-600":"text-destructive"}].map(c=><Card key={c.l}><CardContent className="p-5"><p className="text-sm text-muted-foreground">{c.l}</p><p className={"text-2xl font-bold mt-1 "+(c.c||"")}>{c.v}</p></CardContent></Card>)}
    </div>
