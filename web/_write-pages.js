...
fs.writeFileSync('src/pages/Providers.tsx', `import { useState } from "react";
...
const PO=[{v:"deepseek",l:"DeepSeek"},{v:"qwen",l:"Qwen"},{v:"zhipu",l:"智谱AI"},{v:"moonshot",l:"Moonshot"},{v:"baidu",l:"百度文心"},{v:"xfyun",l:"讯飞星火"},{v:"bytedance",l:"字节豆包"},{v:"tencent",l:"腾讯混元"},{v:"lingyi",l:"零一万物"},{v:"openai",l:"OpenAI"},{v:"anthropic",l:"Anthropic"},{v:"google",l:"Google Gemini"},{v:"openrouter",l:"OpenRouter"},{v:"siliconflow",l:"SiliconFlow"},{v:"other",l:"Other"}];
...
export default function Providers() {
  ...
  const [nm, setNm] = useState(""); const [pr, setPr] = useState("openai"); const [bu, setBu] = useState(""); const [ak, setAk] = useState(""); const [er, setEr] = useState("");
  const cM = useAdminMutation<unknown,Record<string,string>>("post","/providers");
  const uM = useAdminMutation<unknown,{id:string}&Record<string,string>>("put",(v:any)=>"/providers/"+v.id,"/providers");
  const sM = useAdminMutation<{synced:number;message:string},{id:string}>("post",(v:any)=>"/providers/"+v.id+"/sync","/providers");
  const dM = useAdminMutation<unknown,{id:string}>("delete",(v:any)=>"/providers/"+v.id,"/providers");
  ...
  const hsy = async (p:ProviderData) => { setSy(p.id); try{const r=await sM.mutateAsync({id:p.id});alert(r.message)}catch{}}finally{setSy(null)} };
  const hd = async (p:ProviderData) => { if(!confirm("确定停用?"))return; try{await dM.mutateAsync({id:p.id})}catch{} };
  return <div>
    <div className="flex items-center justify-between mb-6"><div><h2 className="text-2xl font-bold">Provider 凭证管理</h2><p className="text-sm text-muted-foreground mt-1">管理上游模型提供商的 API Key 凭证</p></div><Button onClick={oc}><Plus size={16} className="mr-1.5"/>添加凭证</Button></div>
    ...
    {isLoading && <Card><CardContent className="p-12 text-center"><div className="animate-spin w-8 h-8 border-2 border-primary border-t-transparent rounded-full mx-auto mb-3"/><p className="text-muted-foreground">加载中...</p></CardContent></Card>}
    ...
    <div className="space-y-4"><div className="grid grid-cols-2 gap-4"><div className="space-y-2"><Label>名称 *</Label><Input value={nm} onChange={e=>setNm(e.target.value)} placeholder="例如: OpenAI Production"/></div><div className="space-y-2"><Label>提供商 *</Label><Select value={pr} onValueChange={setPr}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{PO.map(o=><SelectItem key={o.v} value={o.v}>{o.l}</SelectItem>)}</SelectContent></Select></div></div><div className="space-y-2"><Label>API Base URL</Label><Input value={bu} onChange={e=>setBu(e.target.value)} placeholder="留空使用默认" className="font-mono"/></div><div className="space-y-2"><Label>API Key {ed?"(留空保持不变)":"*"}</Label><Input type="password" value={ak} onChange={e=>setAk(e.target.value)} placeholder={ed?"留空保持不变":"sk-..."} className="font-mono"/></div></div>
